    ...
    <div className="space-y-8 animate-in fade-in duration-500">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="mt-8 grid grid-cols-2 gap-4 w-full">
            ...
            <div className="bg-muted/50 rounded-2xl p-3">
              ...
              <p className="text-xl font-bold">
                {summary?.recent_anomaly_count || 0}
              </p>
            </div>
          </div>
        </Card>
        ...
        <div className="lg:col-span-2 space-y-6">
            ...
            <div className="space-y-4">
                ...
                <div className="bg-destructive/10 border border-destructive/20 rounded-2xl p-4 flex gap-4">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-destructive-foreground/80 mt-1">
                      Risk level is {summary?.risk_level || "unknown"} with{" "}
                      {summary?.recent_anomaly_count || 0} recent anomaly flags.
                      Review ventilation and threshold breaches before the next
                      cycle.
                    </p>
                  </div>
                </div>
                ...
                <div className="bg-green-500/10 border border-green-500/20 rounded-2xl p-4 flex gap-4">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-green-600/80 mt-1">
                      AI risk scoring currently reports{" "}
                      {summary?.risk_level || "Normal"} conditions with{" "}
                      {summary?.recent_anomaly_count || 0} recent anomaly flags.
                    </p>
                  </div>
                </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="p-4 rounded-2xl border border-border bg-card">
                  ...
                  <p className="text-sm">
                    {summary?.forecast_status === "success"
                      ? `Forecast pipeline is active. Data source: ${summary?.data_source || "unknown"}.`
                      : summary?.forecast_status === "insufficient_data"
                        ? "AI forecast is waiting for a larger live data window."
                        : "AI forecast is currently unavailable."}
                  </p>
                </div>
                <div className="p-4 rounded-2xl border border-border bg-card">
                  <p className="text-xs font-bold text-muted-foreground mb-1">
                  ...
                  <p className="text-sm">
                    {activeAlerts.length} alert{activeAlerts.length === 1 ? "" : "s"} currently open.
                  </p>
                </div>
              </div>
            </div>
          ...
          <Card>
            <CardHeader title="Current Service State" icon={Activity} />
            <div className="space-y-3">
              <div className="flex items-center justify-between p-3 bg-muted/30 rounded-xl">
                <span className="text-sm font-medium">Sensor device</span>
                <Badge
                  variant={deviceSt?.status === "online" ? "success" : "destructive"}
                >
                  {deviceSt?.status || "unknown"}
                </Badge>
              </div>
              <div className="flex items-center justify-between p-3 bg-muted/30 rounded-xl">
                <span className="text-sm font-medium">Analytics pipeline</span>
                <Badge
                  variant={summary?.forecast_status === "success" ? "success" : "warning"}
                >
                  {summary?.forecast_status || "unknown"}
                </Badge>
              </div>
              <div className="flex items-center justify-between p-3 bg-muted/30 rounded-xl">
                <span className="text-sm font-medium">Analytics data source</span>
                <Badge variant="primary">{summary?.data_source || "--"}</Badge>
              </div>
            </div>
          </Card>
        </div>
